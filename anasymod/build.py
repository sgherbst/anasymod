...
class VivadoBuild():
    ...
    def build(self):
        ...
        self.v.create_project(project_name=self.cfg.vivado_config.project_name,
                              ...
                              full_part_name=self.cfg.board.cfg['full_part_name'],
                              force=True)
        ...
        constrs.use_templ(TemplExtClk(ext_clk_pin=self.cfg.board.cfg['clk_pin'],
                                      ext_clk_io_std=self.cfg.board.cfg['clk_io'],
                                      ext_clk_freq=self.cfg.board.cfg['clk_freq']))
        ...
        self.v.use_templ(TemplClkWiz(cfg=self.cfg, target=self.target))
        ...
        constrs.use_templ(TemplDbgHub(dbg_hub_clk_freq=self.cfg.board.cfg['dbg_hub_clk_freq']))
        constrs.write_to_file(cpath)
