...
class VivadoBuild():
    ...
    def build(self):
        ...
        self.v.create_project(project_name=self.prj_cfg.vivado_config.project_name,
                              project_directory=self.target.project_root,
                              full_part_name=self.prj_cfg.board.cfg['full_part_name'],
                              force=True)
        ...
        self.v.use_templ(TemplVIO(outputs=[VioOutput(width=1), VioOutput(width=self.prj_cfg.cfg.dec_bits)],
                                  ip_dir=self.target.ip_dir, ip_module_name=self.prj_cfg.vivado_config.vio_name))
        ...
        self.v.println('launch_runs synth_1 -jobs {0}'.format(self.prj_cfg.vivado_config.num_cores))
        self.v.println('wait_on_run synth_1')

        # extact probes from design
        self.v.use_templ(TemplPROBE_EXTRACT(cfg=self.prj_cfg, target=self.target))

        self.v.run(vivado=self.prj_cfg.vivado_config.vivado, build_dir=self.prj_cfg.build_root, filename=r"synthesis.tcl")

        # append const file with ILA according to extracted probes
        constrs.read_from_file(cpath)
        constrs.use_templ(TemplILA(probe_cfg_path=self.target.probe_cfg_path, inst_name=self.prj_cfg.vivado_config.ila_inst_name))

        constrs.use_templ(TemplDbgHub(dbg_hub_clk_freq=self.prj_cfg.board.dbg_hub_clk_freq))
        constrs.write_to_file(cpath)

        # Open project
        project_path = os.path.join(self.target.project_root, self.prj_cfg.vivado_config.project_name + '.xpr')
        self.v.println(f'open_project "{back2fwd(project_path)}"')

        # launch the build and wait for it to finish
        self.v.println('launch_runs impl_1 -to_step write_bitstream -jobs {0}'.format(self.prj_cfg.vivado_config.num_cores))
        ...
        self.v.run(vivado=self.prj_cfg.vivado_config.vivado, build_dir=self.prj_cfg.build_root, filename=r"bitstream.tcl")

    def run_FPGA(self):
        self.v.use_templ(TemplEXECUTE_FPGA_SIM(cfg=self.prj_cfg, target=self.target))
        self.v.run(vivado=self.prj_cfg.vivado_config.vivado, build_dir=self.prj_cfg.build_root, filename=r"run_FPGA.tcl")